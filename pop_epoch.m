...
% -------------------------
% Set rmBound to true if you to have epochs with boundarys inside removed
rmBound = false;
if rmBound
    disp('pop_epoch(): checking epochs for data discontinuity');
else
    disp('WARNING: pop_epoch manually altered to prevent removal of trials containing boundaries')
end

% if ~isempty(EEG.event) & isstr(EEG.event(1).type)
if ~isempty(EEG.event) & isstr(EEG.event(1).type) & rmBound
    tmpevent = EEG.event;
